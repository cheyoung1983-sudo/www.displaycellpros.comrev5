...
- **Stripe**: `src/lib/stripe.ts` (lazy `getStripe()` client). Checkout is embedded-mode, not hosted-redirect: `app/actions/stripe.ts` (`'use server'` `startCheckoutSession`) is the only Server Action in the codebase — everywhere else uses Route Handlers — and creates the Checkout Session server-side; `StripeCheckoutModal.tsx` calls it directly (no `fetch`) and renders the result via `EmbeddedCheckoutProvider`/`EmbeddedCheckout`. `StripeCheckoutComponent.tsx` and `PaymentCheckout.tsx` are thin wrappers that just open this modal rather than talking to Stripe themselves — don't add a new fetch-based checkout path, extend the modal/action instead. Webhooks: `src/lib/stripeWebhook.ts` + `app/api/stripe/webhook/route.ts` (POST) and `/events` (GET/DELETE) mirror the GitHub webhook pattern in `githubSync.ts` — signature verification via `stripe.webhooks.constructEvent` when `STRIPE_WEBHOOK_SECRET` is set (400 on failure), permissive/unverified parsing when it isn't, in-memory FIFO event log capped at 100. Client publishable key: `NEXT_PUBLIC_STRIPE_PUBLIC_KEY` (falls back to `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`).
